...
TASKS = {
    "MA": {
        "prompt": "retinal microaneurysm",
        "mask_suffix": "_mask.png",
        "output_dir": "sam3_MA_preds"
    },
    "HE": {
        "prompt": "retinal hemorrhage",
        "mask_suffix": "_mask.png",
        "output_dir": "sam3_HE_preds"
    },
    "EX": {
        "prompt": "hard exudate in retina",
        "mask_suffix": "_mask.png",
        "output_dir": "sam3_EX_preds"
    },
    "SE": {
        "prompt": "soft exudate cotton wool spot",
        "mask_suffix": "_mask.png",
        "output_dir": "sam3_SE_preds"
    },
    "OD": {
        "prompt": "optic disc",
        "mask_suffix": "_mask.png",
        "output_dir": "sam3_OD_preds"
    }
}
...
def generate_cmp(pred, gt):
    ...
    # TP 白色
    cmp_img[np.logical_and(pred, gt)] = [255, 255, 255]

    # FP 红色
    cmp_img[np.logical_and(pred, ~gt)] = [0, 0, 255]

    # FN 绿色
    cmp_img[np.logical_and(~pred, gt)] = [0, 255, 0]
...
def run_task(
    ...
    if len(valid) > 0:
        ...
        log.info(
            f"Dice: {valid['dice'].mean():.4f}"
        )
        log.info(
            f"IoU : {valid['iou'].mean():.4f}"
        )

        log.info(
            f"HD95: {valid['hausdorff_distance_95'].mean():.2f}"
        )

        log.info(
            f"ASD : {valid['average_surface_distance'].mean():.2f}"
        )
