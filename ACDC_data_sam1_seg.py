...
CONFIG = {
    ...
    "model": "Models/sam_l.pt", 
    "image_dir": "DataSets/ACDC_pro",
    "output_dir": "DataRes/ACDC_pro/sam1_preds"
}
...
def process_one(model, img_path, out_dir):
    ...
    log.info(f"处理: {rel_path} 唯一名称: {unique_name}")
    start = time.time()
    ...
    if cmp_img is not None:
        cv2.imwrite(os.path.join(out_dir, f"cmp_{unique_name}"), cmp_img)
    else:
        log.warning("[!] 比较失败！")
