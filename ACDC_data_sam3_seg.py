...
CONFIG = {
    ...
    "image_dir": "DataSets/ACDC_pro",
    "output_dir": "DataRes/ACDC_pro/sam3_preds"
}
...
def run_inference(predictor, img_path):
    ...
    results = predictor(text=["heart chamber"])
...
def process_one(predictor, img_path, out_dir):
    # 获取相对路径，保证不同文件夹同名不会覆盖
    rel_path = os.path.relpath(img_path, CONFIG["image_dir"])
    rel_path_flat = rel_path.replace(os.sep, "_")  # 将路径分隔符替换为下划线
    fname = os.path.basename(img_path)
    unique_name = f"{rel_path_flat}"  # 用平铺后的相对路径作为唯一名

    tqdm.write(f"[*] 处理: {rel_path} 唯一名称: {unique_name}")
    ...
    return {
        ...
        "category": os.path.dirname(rel_path),  # 用相对目录作为类别
...
def main():
    ...
    # 使用 tqdm 显示全局进度条
    for path in tqdm(all_images, desc="Processing images", ncols=100):
